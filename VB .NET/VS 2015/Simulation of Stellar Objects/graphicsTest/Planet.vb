Public Class Planet
    ...
    Friend Sub New()
    End Sub
    Friend Sub Init(ByVal pUniverse As Universe, ByVal pLocation As PointF, ByVal pSize As Integer,
                    ByVal pBorderWidth As Integer, ByVal pColor As Color,
                    Optional ByVal pVelX As Double = 0, Optional ByVal pVelY As Double = 0)

        objectUniverse = pUniverse 'Set universe the planet is in.
        ...
        objectSize = pSize 'Init planet size.
        objectRadius = Math.Sqrt(2 * ((objectSize / 2) ^ 2)) + objectBorderWidth 'Init planet radius. Yes the planet is square.
        objectMass = 5.972 * objectSize * 10000 'Init planet mass. 5,972e24 = Earth mass

        objectBorderWidth = pBorderWidth 'Init planet border width.
        objectColor = pColor 'Init planet color.
    ...
    Friend Overrides Sub Paint()
        ...
        If objectUniverse.drawTraj Then
            ...
            If planetTrajectoryPoints.Count > 0 Then
                ...
                If Not objectCenterOfMass.Equals(planetTrajectoryPoints.Last) Then
                    ...
                    If planetTrajectoryPoints.Count = maxPoints Then
                        ...
                        End If
                        planetTrajectoryPoints.Item(planetLastTrajectoryPointIndex) = objectCenterOfMass 'Replace that point with the new one.
                    Else
                        planetTrajectoryPoints.Add(objectCenterOfMass)
                        planetLastTrajectoryPointIndex += 1 'Increment index.
                    End If
                End If
                ...
                For Each point In planetTrajectoryPoints
                    ...
                    If Math.Abs(point.X - previousPoint.X) < 20 And Math.Abs(point.Y - previousPoint.Y) < 20 Then
                        planetGraphics.DrawLine(planetPen, point, previousPoint)
                    End If
                    previousPoint = point
                Next
            Else
                planetTrajectoryPoints.Add(objectCenterOfMass) 'Add first point.
